...
type TemplatesGridProps = {
  ...
  onReorder: (draggedId: string, targetId: string) => void;  // swap by id
  onReorderWorkflow: (draggedId: string, targetId: string) => void;
          ...
          <div key={colIdx} style={{ flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
            {col.map((item) => {
              if (item.kind === "workflow") {
                ...
                const isWfDragOver = dragOverId === `wf-${wf.id}` && draggingIdRef.current !== `wf-${wf.id}`;
                const wfDragStyle: CSSProperties = isWfDragOver ? { outline: "2px dashed #7c3aed", outlineOffset: "2px", opacity: 0.85 } : {};
                return (
                  <article key={wf.id}
                    draggable={!isPinned}
                    onDragStart={() => { draggingIdRef.current = `wf-${wf.id}`; }}
                    onDragEnd={() => { draggingIdRef.current = null; setDragOverId(null); }}
                    onDragOver={(e) => { if (isPinned) return; e.preventDefault(); setDragOverId(`wf-${wf.id}`); }}
                    onDragLeave={() => setDragOverId(null)}
                    onDrop={(e) => {
                      e.preventDefault();
                      const fromKey = draggingIdRef.current;
                      if (!fromKey || isPinned) { setDragOverId(null); return; }
                      if (fromKey.startsWith("wf-") && fromKey !== `wf-${wf.id}`) {
                        // workflow dropped onto workflow
                        onReorderWorkflow(fromKey.slice(3), wf.id);
                      } else if (!fromKey.startsWith("wf-")) {
                        // template dropped onto workflow
                        onReorder(fromKey, wf.id);
                      }
                      setDragOverId(null);
                    }}
                    style={{
              ...
              const isDragOver = dragOverId === template.id && draggingIdRef.current !== template.id;
              const dragStyle: CSSProperties = isDragOver ? {
              ...
              return (
                <article
                  key={template.id}
                  draggable={!isPinned && !isEditing}
                  onDragStart={() => { draggingIdRef.current = template.id; }}
                  onDragEnd={() => { draggingIdRef.current = null; setDragOverId(null); }}
                  onDragOver={(e) => { if (isPinned) return; e.preventDefault(); setDragOverId(template.id); }}
                  onDragLeave={() => setDragOverId(null)}
                  onDrop={(e) => {
                    e.preventDefault();
                    const fromKey = draggingIdRef.current;
                    if (!fromKey || isPinned) { setDragOverId(null); return; }
                    if (fromKey.startsWith("wf-")) {
                      // workflow dropped onto template — reorder workflow relative to template
                      onReorderWorkflow(fromKey.slice(3), template.id);
                    } else if (fromKey !== template.id) {
                      // template dropped onto template
                      onReorder(fromKey, template.id);
                    }
                    setDragOverId(null);
                  }}
                  style={{ ...cardStyle, ...dragStyle, cursor: (!isPinned && !isEditing) ? "grab" : "default" }}>
                  ...
                  {!isEditing && (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "0.1rem", gap: "0.4rem" }}>
                      ...
                      {!isPinned ? (
                        <span style={{ fontSize: "0.62rem", color: mutedText, userSelect: "none", cursor: "grab", letterSpacing: "0.05em" }} title="Drag to reorder">⠿ drag</span>
                      ) : <div />}
